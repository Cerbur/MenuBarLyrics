...
- 支持解析 LRC 风格时间戳，例如 `[00:12.50]`，并会忽略常见 LRC 元信息、应用 `[offset:]` 时间偏移。
- 当歌词没有时间戳时，根据播放进度估算当前歌词行。
- 首次打开会显示设置窗口，可控制是否展示菜单栏歌词，并会记住开关状态；之后启动默认只常驻菜单栏。
...
设置窗口只会在首次启动时自动打开；之后可以从菜单栏图标菜单中选择“打开设置...”。
...
如果 Music.app 没有打开歌词面板，或当前歌曲没有 Apple Music 歌词，菜单栏会显示相应提示。辅助功能读取依赖 Music.app 的界面结构，目前会识别中文“歌词”和英文“Lyrics”歌词面板，macOS 或 Music.app 更新后仍可能需要调整匹配逻辑。
